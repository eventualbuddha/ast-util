...
var Scope = require('ast-types/scope');
...
function callGetIterator(scope, expression) {
  var getIterator = injectHelper(scope.getGlobalScope(), 'getIterator');
...
function getArrayIterator(scope) {
  return injectHelper(scope.getGlobalScope(), 'getArrayIterator');
}
...
function callGetIteratorRange(scope, iterator, index, begin, len) {
  var getIteratorRange = injectHelper(scope.getGlobalScope(), 'getIteratorRange');
...
function callGet(scope, object, property, receiver) {
  var get = injectHelper(scope.getGlobalScope(), 'get');
...
/**
 * Injects the named pre-built helper.
 *
 * @param {Scope} scope
 * @param {string} name
 * @return {Identifier}
 */
function injectHelper(scope, name) {
  return injectShared(
    scope,
    name,
    function() {
      return require('./helpers/' + name)(scope);
    }
  );
}
